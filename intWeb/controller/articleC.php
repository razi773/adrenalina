<?php
    ...
    Class articleC {
        ...
        {
            $requete = "select * from ARTICLE where titre like '%$titre%'";
            $config = config::getConnexion();
            try {
                $querry = $config->prepare($requete);
                $querry->execute();
       ...
       function recherchearticle($id)
        {
            $requete = "select * from article where id like '%$id%'";
            $config = config::getConnexion();
            try {
                $querry = $config->prepare($requete);
                $querry->execute();
        ...
        function ajouterarticle($article)
        {
            ...
            try {
                ...
                INSERT INTO article
                (id,titre,description,img,auteur,date)
                VALUES
                (:id,:titre,:description,:img,:auteur,:date)
                ');
                $querry->execute([
        ...
        function modifierarticle($article)
        {
            ...
            try {
                $querry = $config->prepare('
                UPDATE article SET
              titre=:titre,description=:description,img=:img,auteur=:auteur,date=:date
